...
test_that("all methods", {
  ...
  true_mean_fun <- function(xx) sin(10 * apply(xx, 1, mean) + 1) + sapply(xx[, 3], function(x) rnorm(1, mean=0,sd=x))
  ...
  mod <- deepregression(
    y = y,
    data = data,
    list_of_formulae = list(loc = ~ X3 + d(X1) + g(X2), scale = ~X3),
  ...
  q95 = quantile(mod, data, 0.95)
  q05 = quantile(mod, data, 0.05)
...
# # FIXME Test use case with tensors dataset OR remove check for it in
# # fit_fun.R L67
# test_that("all methods - tfdata dataset", {
#   n <- 1500
#   deep_model <- function(x) x %>%
#     layer_dense(units = 2L, activation = "relu", use_bias = FALSE) %>%
#     layer_dense(units = 1L, activation = "linear")
